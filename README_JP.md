...
よく知られているように、Windows XP は 2014 年に Microsoft からの公式サポートを失いました。Posready 2009 と呼ばれる特別バージョンがあり、2019 年にサポートが終了しました。これは必然的に 6 年前に発生しました。したがって、Windows XP 用のドライバーは 2012 年から 2016 年にかけて開発されなくなり、NVIDIA が Windows XP のサポートを終了した最後の大手メーカーとなりました。同時に、ソフトウェア開発者もほぼ同じ時期に Windows XP のサポートを終了し始めました。したがって、懐かしさから、新しいバージョンよりも優れていると考えているため、またはその他の理由で Windows XP を使用したい場合、いくつかのケースを除いて、現在のハードウェアと現在のソフトウェアで使用することはほぼ不可能です。具体的には、MyPal 68/78 と Supermium があります。これは、XP から Windows 10 まで動作できる Chromium の変更バージョンです。それ以外では、実質的にソフトウェアは互換性がありません。そのため、この問題を解決するために One-Core-API が作成されました: Windows XP および Server 2003 でソフトウェアとハードウェアの互換性を再確立するためです。
ただし、1 つの大きな問題があります: XP と Windows Vista の間の飛躍は大きく、XP には Windows Vista で導入された新しいテクノロジの 90 ～ 95% が欠けています（Microsoft によって移植されたいくつかを除く）。これにより、課題が大幅に増加し、多くのエラーが発生する可能性があります。したがって、One-Core-API は安定性の問題を抱える可能性があり、一部のシステムを破損する可能性があります。また、テスト環境の問題もあります。発生する可能性のある無限の可能性、またはスタッフの不足（私は主な開発者であり、Murak、Svyatpro、最近では CoccodrillooXDS など、他の数人が私を助けています）のいずれかのために、問題を予測することは不可能です。
したがって、One-Core-API は、ゲームを含む多くの最新のアプリケーションを実行できる唯一のプロジェクトであることを理解してください。しかし、それは集中的な開発中であり、多くのエラーが発生します。我慢してください; エラーを見つけた場合は、文句を言う代わりにここで報告し、あなたが望み、良い心を持っているなら、Windows XP コミュニティの成長を助けてください。ありがとうございました。
. 
> [!WARNING]
> OCA は公式にいかなるアンチウイルス（AV）もサポートしていません。OCA を使用したい場合は、事前に AV をアンインストールしてください。
